...
public interface ITopic extends Serializable {
    ...
    // Método padrão que notifica todos os consumidores sobre uma nova mensagem, rodando de forma assíncrona

        consumers().forEach( consumer -> {
        // Para cada consumidor registrado no tópico

           CompletableFuture<Boolean> completableFuture = CompletableFuture
                   .supplyAsync(() -> consumer.consume(message));
                   // Cria um `CompletableFuture` para consumir a mensagem de forma assíncrona, 
                   // permitindo que o processamento da mensagem não bloqueie a execução do programa
